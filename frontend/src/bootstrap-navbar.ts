...
const navbarInit = () => {
  ...
  const navbar = document.querySelector<HTMLElement>(Selector.NAVBAR);
  // responsive nav collapsed
  navbar!.addEventListener('click', (e) => {
    if (
      (e.target as HTMLElement).classList.contains('nav-link') &&
      window.innerWidth < utils.getBreakpoint(navbar)!
    ) {
      navbar!.querySelector<HTMLElement>(Selector.NAVBAR_TOGGLER)!.click();
    }
  });
  ...
  if (navbar) {
    ...
    const navbarCollapse = navbar.querySelector<HTMLElement>(Selector.NAVBAR_COLLAPSE)!;
    const allColors: Record<string, string> = { ...utils.colors, ...(utils.grays as Record<string, string>) };
    ...
    window.addEventListener(Events.SCROLL, () => {
      ...
      let alpha = (scrollTop / windowHeight) * 0.75;
      // Add class on scroll
