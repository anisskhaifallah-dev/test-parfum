...
export function initProductPage(): void {
  ...
  if (!product) {
    root.innerHTML = `
      <div class="col-12 text-center py-8">
        <h2 class="fw-normal">We couldn't find that fragrance.</h2>
        <a class="btn btn-dark mt-3" href="index.html">Back to Shop</a>
      </div>
    `;
  ...
  root.innerHTML = `
    <div class="col-lg-6">
      <img class="pdp-image" src="${product.image}" alt="${product.name}" width="600" height="750" />
    </div>
    <div class="col-lg-6">
      <span class="shop-card-family">${product.family} &middot; ${genderLabel(product.gender)}</span>
      <h1 class="fw-normal mt-2 mb-3">${product.name}</h1>
      <p class="text-700">${product.blurb}</p>
      <div class="notes-pyramid">
        <div class="notes-row"><span class="notes-label">Top</span><span class="notes-values">${notes.top.join(', ')}</span></div>
        <div class="notes-row"><span class="notes-label">Heart</span><span class="notes-values">${notes.heart.join(', ')}</span></div>
        <div class="notes-row"><span class="notes-label">Base</span><span class="notes-values">${notes.base.join(', ')}</span></div>
      </div>
      <div class="size-picker" id="size-picker">
        ${product.sizes.map((s) => `<button type="button" class="size-option ${s.ml === selectedMl ? 'active' : ''}" data-ml="${s.ml}">${s.label}</button>`).join('')}
      </div>
      <div class="fw-bold fs-3 mb-4" id="pdp-price">${priceFor(selectedMl)} DH</div>
      ${!product.available ? '<p class="text-danger fw-bold mb-2">Currently unavailable</p>' : ''}
      <div class="d-flex gap-2">
        <button type="button" class="btn btn-lg btn-dark flex-grow-1" id="add-to-cart" ${product.available ? '' : 'disabled'}>${product.available ? 'Add to Cart' : 'Out of Stock'}</button>
        <button type="button" class="shop-card-wishlist ${isWishlisted(product.id) ? 'active' : ''}" style="position:static;width:52px;height:52px;flex-shrink:0;" id="pdp-wishlist" aria-label="Toggle wishlist">${heartIcon(isWishlisted(product.id))}</button>
      </div>
      <div id="add-confirm" class="text-success mt-2 fs--1 d-none">Added to cart.</div>
    </div>
  `;
