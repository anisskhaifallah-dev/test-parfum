...
// Comma-separated, e.g. "657209251,7758128102" - lets more than one person get notified
// without a code change every time someone new needs to be added.
  ...
  await Promise.all(
    chatIds.map(async (chatId) => {
      try {
        ...
        console.error(`Failed to send order Telegram notification to chat ${chatId}`, err);
      }
    })
  );
}
